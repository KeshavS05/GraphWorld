...
class GraphWorld():
    ...
    def __init__(self, parse_args=False, html=True, threeD=False, length=0, height=0, depth=2, adj=np.array([]),
    ...
    def add_edges(self):
        ...
        self.s_adj = [[[] for _ in range(len(self.adj))] for _ in range(len(self.adj))]
            ...
            for j in range(i, self.num_nodes):
                ...
                if weight == 0:
                    ...
                    if weight == 1:
                        ...
                        self.s_adj[j][i] = [weight, ind1, ind2]
                        
        for row in self.s_adj:
            print(row)
                        # self.get_node(ind1).supporting_edges.append((ind, i, j))
                        # self.s_adj[i][j] = [weight, (ind1, ind2)]
    ...
    def set_weights_of_edges(self):
        ...
        for x, y in self.supporting_edge_dict:
            ...
            if self.get_node(ind1).num_agents > 0 and self.get_node(ind2).num_agents > 0:
    ...
    def reset(self):
        ...
        if self.threeD:
            ...
            self.R_2d = twoD_Render(self.html, self.length, self.height, self.depth, self.G, self.agents, self.weight_before, self.weight_after, self.time_step)
    ...
    def step(self, actions=None):
        ...
        if done and not self.html and not self.threeD:
            plt.show()
        ...
        for i in range(len(self.adj)):
            for j in range(len(self.adj[i])):
                if i == j and j==0:
                    ...
                    edge_number+= 1
                edge_index_map[(i, j)] = [edge_number, self.adj[i][j]]
    ...
    def render(self, image_size=800, grid_line_color=(87, 89, 93), grid_tick_val=1, grid_line_type=cv2.LINE_AA, default_edge_color=(0, 255, 0), active_support_edge_color=(255, 174, 66), inactive_support_edge_color=(128, 0, 128), node_color=(255, 255, 255), node_border_color=(203, 192, 255), terminating_node_color=(0, 0, 255), supporting_node_color=(255, 0, 0), agent_color=(255, 255, 0), agent_label_text_color=(0, 0, 0)):
