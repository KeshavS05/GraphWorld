...
class Render():
    ...
    def __init__(self, length, height, G, agents, type_of_weighted) -> None:
        self.length = length
        self.height = height
        self.scale_length = 0
        self.scale_height = 0
    ...
    def render_edges(self, img, default_edge_color, active_support_edge_color, inactive_support_edge_color,):
        for x, y in self.type_of_weighted:
            if self.type_of_weighted[(x, y)] == 1:#w == self.weight_before:
            ...
            elif self.type_of_weighted[(x, y)] == 0:#w == 1:
            ...
            elif self.type_of_weighted[(x, y)] == 2:#else:
    ...
    def render_nodes(self, img, image_size, node_color, node_border_color, terminating_node_color, supporting_node_color):
        for label in list(self.G.nodes.keys()):
            ...
            cv2.circle(img, n.scale_coords(self.scale_length, self.scale_height), int(image_size/(len(self.nodes) * 10)), node_border_color, 5)
            if n.terminating:
                cv2.circle(img, n.scale_coords(self.scale_length, self.scale_height), int(image_size/(len(self.nodes) * 10)), terminating_node_color, -1)
            elif n.supporting:
                cv2.circle(img, n.scale_coords(self.scale_length, self.scale_height), int(image_size/(len(self.nodes) * 10)), supporting_node_color, -1)
            else:
                cv2.circle(img, n.scale_coords(self.scale_length, self.scale_height), int(image_size/(len(self.nodes) * 10)), node_color, -1)
    ...
    def render_agents(self, img, image_size, agent_color, agent_label_text_color):
        ...
        for agent in self.agents:
            ...
            pt1 = (x - int(image_size/(len(self.nodes) * 12)) + shift_factor_x, y + int(image_size/(len(self.nodes) * 12)) + shift_factor_y)
            pt2 = (x + int(image_size/(len(self.nodes) * 12)) + shift_factor_x, y + int(image_size/(len(self.nodes) * 12)) + shift_factor_y)
